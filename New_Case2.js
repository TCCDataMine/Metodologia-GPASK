...
function getPosicao(grupo) {
  ...
  if(grupo.length > 0) {
    ...
    // Retorna os piores alunos da melhor hardskill do grupo
    ...
    //coleta a hardskill com menos influcia
    const hsHighInfluence = hsInfluence.indexOf(Math.min(...hsInfluence));

    //coleta os melhores alunos das HardSkills menos influentes
    const hsBStudent = turma.alunos.map((aluno) => aluno.hardskills[hsActivity[hsHighInfluence]].nota);

    //Define melhor aluno da hardSkill menos influente
